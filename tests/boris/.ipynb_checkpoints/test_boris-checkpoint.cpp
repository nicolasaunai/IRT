...
void uniform_bz()
{
    ...
    while (time < final_time)
    {
        ...
        // Save updated positions and velocities AFTER the push
    ...
    {
        HighFive::File file("uniform_bz.h5", HighFive::File::Truncate);
...
void drift_ey()
{
    ...
    // Save results to HDF5
    {
        HighFive::File file("drift_ey.h5", HighFive::File::Truncate);
        file.createDataSet("/x", x);
        file.createDataSet("/vx", vx);
        file.createDataSet("/vy", vy);
        file.createDataSet("/vz", vz);
